...
namespace Console.Runners
{
    class MultiplicationPairsRunner
    {
        public static void Run(List<int> numbers, int x, IConsole console)
        {
            ...
            if (tuples.Count() > 0)
            {
                console.Out.WriteLine($"{Environment.NewLine}Pairs of numbers that can be multiplied to get {x}:");
            ...
            {
                console.Out.WriteLine($"{Environment.NewLine}None of the numbers can be multiplied to get {x}");
            }
        }
    }
}
